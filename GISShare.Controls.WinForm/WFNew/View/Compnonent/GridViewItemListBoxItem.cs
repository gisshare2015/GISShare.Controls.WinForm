...
namespace GISShare.Controls.WinForm.WFNew.View
{
        ...
        IGridViewItemListBox, IGridViewItemListBoxEvent
    {
        ...
        public GridViewItemListBoxItem()
        {
        ...
        {
            GridViewItemListBoxItem baseItem = new GridViewItemListBoxItem();
            return baseItem;
        }
        #endregion
        ...
        [Browsable(true), DefaultValue(false), Description("显示列标题"), Category("外观")]
        ...
        void IColumnViewObjectHelper.InsertColumnViewItem(int index)
        {
            foreach (IRowCellViewItem one in this.m_ViewItemCollection)
            {
                ...
                {
                    case RowCellViewStyle.eSystemRow:
                        one.Items.Insert(index, this.CreateCellViewItem(CellViewStyle.eSystem));
                        break;
                    case RowCellViewStyle.eSingleCellRow:
                        if (one.List.Count <= 0)
                        {
                            one.Items.Insert(index, this.CreateCellViewItem(CellViewStyle.eSingleCell));
                        }
                        break;
                    default:
                        break;
        ...
        void IColumnViewObjectHelper.ExchangeColumnViewItem(int index1, int index2)
        {
            foreach (IRowCellViewItem one in this.m_ViewItemCollection)
            {
                ...
                {
                    case RowCellViewStyle.eSystemRow:
                        one.List.ExchangeItem(index1, index2);
                        break;
                    case RowCellViewStyle.eSingleCellRow:
                        break;
                    default:
        ...
        private void MSPaint(MessageInfo messageInfo)
        {
            ...
            if (this.ShowColumnTitle)
            {
                ...
                if (this.ShowRowHeader)
                {
                    ...
                    //
                    clipRectangle = Rectangle.Intersect(clipRectangle, e.ClipRectangle);//Key
                    //
                ...
                if (this.m_OperationStyle == 4)//CanResizeColumnWidth
                {
                    if (this.m_SplitLineForm != null)
                    {
                        ...
                        if (columnViewItem != null)
                        {
                            ...
                            this.Invalidate(this.ViewRectangle);
                        }
                    }
                    messageInfo.CancelPreEvent = true;//取消事件 1
                    return;
