...
{
    public class RowColumnTitleViewItem : TitleViewItem, 
        IRowViewItem,
        ...
        IViewItemOwner, IViewItemOwner2, IViewList
    {
        ...
        public RowColumnTitleViewItem()
            : base()
        {
            this.m_ViewItemCollection = new TitleViewItemCollection(this);
        }

        public RowColumnTitleViewItem(string text)
            : this()
        {
            this.Text = text;
        }

        public RowColumnTitleViewItem(string name, string text)
            : this()
        {
            this.Name = name;
            this.Text = text;
        ...
        public RowColumnTitleViewItem(string name, string text, Font font)
            : this()
        {
            this.Name = name;
            this.Text = text;
        ...
        bool IFlexibleRowViewItem.CanExchangeItem
        {
            get { return false; }
            set { }
        }
        ...
        #region 属性
        private TitleViewItemCollection m_ViewItemCollection;
        [Browsable(true),
        ...
        Category("子项")]
        public TitleViewItemCollection ViewItems
        {
        ...
        public override int Width
        {
            ...
            set
            {
                ...
                {
                    int iCount = 0;
                    foreach (TitleViewItem one in this.ViewItems)
                    {
                        if (one.Visible) iCount++;
                    }
                    if (iCount > 0)
                    {
                        int iZL = iW / iCount;
                        ...
                        foreach (TitleViewItem one in this.ViewItems)
                        {
                            if (one.Visible) { one.Width += iYS; break; }
                        }
                    }
                }
            }
                    ...
                    if (pSizeViewItem != null)
                    {
                        ...
                        if (iW != 0)
                        {
                            if (this.IsFilled)
                            {
                                ...
                                if (pSizeViewItem != null) { pSizeViewItem.Width -= iW; }
                            }
                            this.Invalidate(this.DisplayRectangle);
                        }
                    }
        ...
        public override Size MeasureSize(Graphics g)
        {
            if (this.ViewItems.Count > 0)
            {
                ...
                foreach (TitleViewItem one in this.ViewItems)
                {
                    ...
                    if (iH < size.Height) iH = size.Height;
                    iW += size.Width;
                }
                return this.IsFilled ? new Size(iW, iH) : new Size(iW, this.Height > iH ? this.Height : iH);
            }
            //
