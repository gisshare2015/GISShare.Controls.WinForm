...
{
    [ToolboxItem(true)]
    public class GridNodeViewItemTree : BaseItemControl, 
        ...
        IGridNodeViewItemTree, IGridNodeViewItemTreeEvent
    {
        ...
        public void SetDataSource(object objDataSource, string objNodeID, string objParentNodeID, string[] strRootParentIDArray)
        {
            ...
            {
                if (this.SetDataSource(objDataSource as DataTable, objNodeID, objParentNodeID, strRootParentIDArray)) return;
                if (this.SetDataSource(objDataSource as IList, objNodeID, objParentNodeID, strRootParentIDArray)) return;
            }
        }
        private bool SetDataSource(DataTable dataTable, string objNodeID, string objParentNodeID, string[] strRootParentIDArray)
        {
            ...
            foreach (DataRow one in dataTable.Rows)
            {
                ...
                if (objValue != null) { strValue = objValue.ToString().Trim(); }
                if (!strRootParentIDArray.Contains(strValue)) continue;
                //
                ...
                this.m_NodeViewItemCollection.Add(rowViewItem);
                //
                this.SetDataSource_DG(rowViewItem, dataTable, objNodeID, objParentNodeID, strRootParentIDArray);
            }
            //
            return true;
        }
        private void SetDataSource_DG(RowNodeCellViewItem rowNodeViewItem, DataTable dataTable, string objNodeID, string objParentNodeID, string[] strRootParentIDArray)
        {
            foreach (DataRow one in dataTable.Rows)
            {
                object objValue = one[objParentNodeID];
        ...
        private bool SetDataSource(IList list, string objNodeID, string objParentNodeID, string[] strRootParentIDArray)
        {
            ...
            if (typeArray.Length > 0)
            {
                ...
                foreach (object one in list)
                {
                    ...
                    this.m_NodeViewItemCollection.Add(rowViewItem);
                    //
                    this.SetDataSource_DG(rowViewItem, list, type, objNodeID, objParentNodeID, strRootParentIDArray);
                }
            }
            return true;
        }
        private void SetDataSource_DG(RowNodeCellViewItem rowNodeViewItem, IList list, Type type, string objNodeID, string objParentNodeID, string[] strRootParentIDArray)
        {
            foreach (object one in list)
            {
                PropertyInfo propertyInfo = type.GetProperty(objParentNodeID);
