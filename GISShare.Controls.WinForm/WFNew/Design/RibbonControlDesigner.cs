...
{
    public class RibbonControlDesigner : ControlDesigner
    {
        ...
        public override void Initialize(IComponent component)
        {
            ...
            //
            this.m_RibbonControl = base.Component as RibbonControl;
            if (this.m_RibbonControl == null)
            {
                this.DisplayError(new ArgumentException("RibbonControl == null"));
                return;
            }
            //
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                //
                foreach (RibbonPageItem one in this.m_RibbonControl.RibbonPages)
                {
                    one.Dispose();
                }
            ...
            if (this.m_RibbonControl.Created)
            {
                switch (m.Msg)
                {
                    ...
                    case (int)GISShare.Win32.Msgs.WM_RBUTTONDOWN://0x204
                        if (this.m_RibbonControl.Handle != m.HWnd) return;
        ...
        private bool SelectCompnentMouseDown(Point point)
        {
            ...
            //
            Rectangle rectangle = this.m_RibbonControl.PageDisplayRectangle;
            if (rectangle.Contains(point))
            {
                point = new Point(point.X - rectangle.X, point.Y - rectangle.Y);
                if (this.SelectCompnentMouseDown_DG(this.m_RibbonControl.SelectedRibbonPage, point)) return true;
                //foreach (BaseItem one in this.m_RibbonControl.RibbonPages)
                //{
                //    if (this.SelectCompnentMouseDown_DG(one as RibbonPageItem, point)) return true;
                //}
            }
            //
            return false;
        ...
        private bool SelectCompnentMouseDown_DG(RibbonPageItem ribbonPageItem, Point point)
        {
            if (ribbonPageItem == null) return false;
            //
            foreach (BaseItem one in ribbonPageItem.BaseItems)
            {
            ...
            //
            if (ribbonPageItem.PreButtonVisible &&
                ribbonPageItem.PreButtonRectangle.Contains(point))
            {
                if (ribbonPageItem.PreButtonIncreaseIndex) ribbonPageItem.TopViewItemIndex++;
                else ribbonPageItem.TopViewItemIndex--;
            }
            if (ribbonPageItem.NextButtonVisible &&
                ribbonPageItem.NextButtonRectangle.Contains(point))
            {
                if (ribbonPageItem.PreButtonIncreaseIndex) ribbonPageItem.TopViewItemIndex--;
                else ribbonPageItem.TopViewItemIndex++;
            }
        ...
        private bool SelectCompnentMouseUp(Point point)
        {
            ...
            {
                point = new Point(point.X - rectangle.X, point.Y - rectangle.Y);
                if (this.SelectCompnentMouseUp_DG(this.m_RibbonControl.SelectedRibbonPage, point)) return true;
                //foreach (BaseItem one in this.m_RibbonControl.RibbonPages)
                //{
                //    if (this.SelectCompnentMouseUp_DG(one as RibbonPageItem, point)) return true;
                //}
            }
            //
            return false;
                    ...
                    if (tabButtonItem.DesignMouseClickRectangleContainsEx(point))
                    {
                        ...
                        if (pSelectionService != null)
                        {
                            ...
                            if (this.ExchangeRibbonPage(this.m_Item1 as RibbonPageTabButtonItem, this.m_Item2 as RibbonPageTabButtonItem))
                            {
                                ...
                                if (tabButton != null)
                                {
                                    RibbonPageItem ribbonPage = tabButton.pTabPageItem as RibbonPageItem;
                                    if (ribbonPage != null)
                                    {
                                        pSelectionService.SetSelectedComponents(new Component[] { ribbonPage as Component }, SelectionTypes.Primary);
                                        this.m_RibbonControl.Refresh();
        ...
        private bool SelectCompnentMouseUp_DG(RibbonPageItem ribbonPageItem, Point point)
        {
            if (ribbonPageItem == null) return false;
            //
            foreach (BaseItem one in ribbonPageItem.BaseItems)
            {
                ...
                if (one.DesignMouseClickRectangleContainsEx(point))
                {
                    ...
                    {
                        this.m_Item2 = one;
                        if (ribbonPageItem.BaseItems.ExchangeItem(this.m_Item1, this.m_Item2))
                        {
                            pSelectionService.SetSelectedComponents(new Component[] { this.m_Item1 as Component }, SelectionTypes.Primary);
                            this.m_RibbonControl.Refresh();
                            return true;
        ...
        //    if (host != null)
        //    {
        //        RibbonPageItem ribbonPage = host.CreateComponent(typeof(RibbonPageItem)) as RibbonPageItem;
        ...
        private class BaseItemCollectionEditerFormEx : WFNew.Design.BaseItemCollectionDesignerForm
        {
            ...
            public BaseItemCollectionEditerFormEx(RibbonControl ribbonControl)
                : base(ribbonControl)
            {
                this.m_RibbonControl = ribbonControl;
                //
            ...
            protected override Dictionary<string, Type[]> CreateNewItemTypesDictionary()
            {
                ...
                typeCreateNewItemTypesDictionary.Add
                    (
                    "GISShare.Controls.WinForm.WFNew.RibbonControl+RibbonQuickAccessToolbarItemEx",
                    new Type[] { typeof(BaseButtonItem), typeof(CheckButtonItem), typeof(DropDownButtonItem), typeof(SplitButtonItem), typeof(ButtonItem), typeof(LabelItem), typeof(LinkLabelItem), typeof(SeparatorItem) }
                    );
                typeCreateNewItemTypesDictionary.Add
                    (
                    "GISShare.Controls.WinForm.WFNew.RibbonControl+RibbonPageContentContainerItem",
                    new Type[] { typeof(TextBoxItem), typeof(ComboBoxItem), typeof(ComboTreeItem), typeof(BaseButtonItem), typeof(CheckButtonItem), typeof(DropDownButtonItem), typeof(SplitButtonItem), typeof(ButtonItem), typeof(LabelItem), typeof(LinkLabelItem), typeof(SeparatorItem) }
                    );
                typeCreateNewItemTypesDictionary.Add
                    (
                    "GISShare.Controls.WinForm.WFNew.RibbonControl+RibbonPageCollection",
